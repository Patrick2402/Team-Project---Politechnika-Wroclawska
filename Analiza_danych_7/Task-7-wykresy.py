...
df = pd.read_csv('Analiza_danych_7/Task-7_result.csv')
...
for year in df['year'].unique():
    ...
    for index, row in year_data.iterrows():
        if row['baseScore'] > 0:
            x_index = int(row['baseScore'] * 10)
            y[x_index] += row['vectorString']

    fig, ax = plt.subplots(figsize=(12, 5))
    ax.bar(x, y, width=0.1, edgecolor='white', linewidth=0.5)

    # Set the title and axis labels
    ax.set_title(f'Liczba kombinacji flag dla danego base score dla roku {year} (CVSS 2)')
    ...
    plt.savefig(f'Analiza_danych_7/wykresy/CVSSv2_{year}.png')
